...
import { PizzaType, OrderItem } from '../types/pizza';
...
const OrderCreation: React.FC<OrderCreationProps> = ({
  ...
  const handleQuantityChange = (pizzaId: number, newQuantity: number) => {
    if (newQuantity <= 0) {
      onRemoveFromOrder(pizzaId);
    } else {
      // Find the pizza and create updated order item
      const pizza = menu.find(p => p.id === pizzaId);
      if (pizza) {
        try {
          const updatedItem = OrderService.createOrderItem(pizza, newQuantity);
          const newOrderItems = orderItems.map(item =>
            item.pizza.id === pizzaId ? updatedItem : item
          );
          // This would need to be handled by parent component
          // For now, we'll just remove and re-add
          onRemoveFromOrder(pizzaId);
          onAddToOrder(pizzaId, newQuantity);
        } catch (error) {
                    ...
                    <div className="item-controls">
                      <div className="quantity-controls">
                        <button
                          ...
                          onClick={() => handleQuantityChange(item.pizza.id, item.quantity - 1)}
                        >
                          −
                        </button>
